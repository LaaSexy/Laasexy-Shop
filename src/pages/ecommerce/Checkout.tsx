...
import { HomeOutlined, InfoCircleOutlined, LeftOutlined, PhoneOutlined, ShoppingCartOutlined } from '@ant-design/icons';
...
const Checkout = () => {
  ...
  const CheckoutSuccessPage = () => (
    <div className="bg-white dark:bg-black flex justify-center items-center min-h-svh">
      <div className="flex flex-col items-center rounded-xl bg-white dark:bg-black p-6 md:p-8 lg:p-10 shadow-lg">
        <div className="text-black animate-pulse mb-8">
          <img src="/assets/images/checkout.png" alt="Checkout Successfully" className="size-96" />
        </div>
      ...
      <div className="flex min-h-screen flex-col">
        <div className="relative flex min-h-screen max-w-full flex-col bg-white dark:bg-black">
          ...
          <header className="sticky top-0 z-10 flex w-full items-center justify-between rounded-b-lg bg-white py-2 shadow-md dark:shadow-[0_4px_6px_rgba(255,255,255,0.1)] dark:bg-black sm:py-2">
            <Button
              className="float-left flex items-center justify-center border-none p-5 text-2xl shadow-none hover:text-black dark:hover:text-white"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="borer bg-white px-4 py-4 rounded-md dark:bg-black shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)]">
                ...
                <List
                  ...
                  renderItem={(item: any) => (
                    <List.Item>
                      <div className="flex w-full items-center justify-between rounded-lg border bg-white p-1 shadow-sm dark:border-gray-700 hover:shadow-[0_3px_10px_rgb(0,0,0,0.2)] transition-shadow duration-300 dark:bg-slate-900">
                        <div className="flex items-start">
                          <Avatar
                            src={IMAGE_PATH + (item?.imageUrl || 'default-image')}
                            alt={item?.name || 'Avatar'}
                            className="ml-1 mr-2 mt-1 mb-1 size-16 rounded-md sm:size-16"
                          />
                          <div className="w-48 text-gray-700 dark:text-white sm:mt-2">
                            ...
                            <div className="flex items-center space-x-4">
                              <Button
                                shape='circle'
                                className="flex items-center text-base sm:text-xl border-violet-500 bg-violet-500 text-white hover:!text-white font-semibold justify-center w-7 h-7 sm:w-7 sm:h-7 dark:border-slate-600 dark:text-slate-300"
                                disabled
                              >
                                -
                              </Button>
          ...
          <footer className="fixed bottom-0 left-0 flex w-full items-center justify-center rounded-t-2xl bg-white py-4 shadow-[0px_-4px_6px_rgba(0,_0,_0,_0.1)] dark:shadow-[0_-4px_6px_rgba(255,255,255,0.1)] dark:bg-slate-800 sm:py-6">
            <button
              ...
              className={`mb-2 flex w-11/12 items-center justify-center rounded-3xl border-none bg-gradient-to-r from-violet-500 to-indigo-600 p-3 text-center text-white transition-colors duration-300 shadow-lg sm:mx-24 ${
                items.length <= 0 ? 'opacity-50 hover:opacity-none cursor-not-allowed' : ''
                }`}
