   ...
   const GridContent = () => (
     <div className="mb-24 w-full overflow-hidden bg-white dark:bg-black">
       <List
         ...
         grid={{ xs: 2, sm: 2, lg: 3, xl: 4, xxl: 5, gutter: 10 }}
         renderItem={(item: any) => (
           <List.Item>
             <div
    ...
    <div className="w-full bg-white dark:bg-black">
      <List
        ...
        renderItem={(item: Item) => (
          <List.Item>
            <div className="flex w-full justify-between rounded-md border shadow-sm dark:border-gray-700 dark:bg-slate-900">
                ...
                <div className="flex w-full min-w-full max-w-full items-start justify-start gap-4 overflow-x-auto whitespace-nowrap sm:pl-5 sm:pr-14">
                    ...
                    {shopV2Data?.subCategories?.map((subCategory: any) => (
                      <li key={subCategory._id} className="list-none">
                        <Button
                          ...
                          className={`my-3 flex items-center justify-center rounded-md border border-[#DBD5D5] !p-5 text-base text-white hover:!border-white hover:bg-violet-500 hover:!text-white sm:my-4 ${
                            selectedCategory === subCategory._id
                              ? ' bg-purple-700 text-white hover:!text-white'
                              : 'bg-transparent'
                          }`}
                  ...
                  <div className="absolute right-0 mt-3 sm:mt-3 flex h-[50px] w-[50px] items-center justify-center rounded-md bg-violet-500">
                    <Button
                      size="large"
                      onClick={() => setShowCart(!showCart)}
                      className="flex items-center justify-center bg-violet-500 text-xl text-white hover:!text-white hover:!border-white dark:border-none"
                      aria-label="Open Options"
                    >
                      {showCart ? <UnorderedListOutlined /> : <AppstoreOutlined />}
                    </Button>
            ...
            <main className="mb-4 flex-1 px-2 mt-4 sm:px-4">
              {Object.entries(groupedItems).map(([categoryId, items]) => (
                <div key={categoryId} className="bg-white dark:bg-slate-800 rounded-sm shadow-sm mb-4">
                  ...
                  <div className="p-4">
                    {showCart ? <GridContent/> : <ListContent items={items} />}
                  </div>
                </div>
              ))}
            </main>
